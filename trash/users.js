...
router.post("/borrow", async (req, res, next) => {
  try {
    const book = await bookSchema.findOne({ isbn: req.body.isbn })
    if (book == null) {
    ...
    const updatedBook = await bookSchema.findById(book.id)
...
router.get("/borrowed-books", async (req, res, next) => {
  try {
    const result = await bookSchema.find({ "borrowedBy": { "$in": req.session.userId } })
